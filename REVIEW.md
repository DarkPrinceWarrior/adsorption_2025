...
Усилить UQ: бутстрап/оут-оф-фолд ансамбли, калибровка σ, включение σ в BO (минимизация μ+λσ или EI/PI).
  ...
  - Стехиометрические/физические ограничения не проверяются на этапе подготовки данных: R_molar только вычисляется (src/
    adsorb_synthesis/data_processing.py:237-257), STOICHIOMETRY_TARGETS нигде не применяются в валидации/обучении — некорректные соотношения могут попасть в датасет (в BO они есть).
  - UQ всё ещё на одном train/test-сплите: ансамбль — 5 бутстрап-моделей на одном разбиении (scripts/train_forward_model.py:96-
    200), нет k-fold/OoF-блендинга, калибраторы строятся только на OOB этого же сплита.
